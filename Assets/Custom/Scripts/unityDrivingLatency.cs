...
public class unityDrivingLatency : MonoBehaviour
{
	...
	public float reverse; //changed this to public float
	public float neutral; //changed this to a public float
	public float acceleration;
	public float neutralBrake;
	public float driveAccel; //link acceleration to drive gear
	public float brake;
	...
	public float yaw_hp;
	[Tooltip("Default and max at 1")]
	public float bumpyTestPitch = 1f;

	[Tooltip("Default and max at 1")]
	public float bumpyTestRoll = 1f;
	...
	public float cruiseAccel;
	public float cruiseSpeed;
	public Text cruiseDisplay;
	//public 
	...
	void Awake()
	{
		cruiseDisplay.gameObject.SetActive(false);
	...
	void Update()
	{
		...
		neutralBrake = Logitech.neutral ? 1f : 0f;

		if (!Logitech.cruise)
		{
			//driveAccel = (!Logitech.neutral && !Logitech.reverse) ? 0.5f : 0f;
			//normal acceleration ---------------------------
			if (!Logitech.isParked)
			{
				acceleration = (defaultAccel + (Logitech.accel + accelTest) * (1 - defaultAccel)) * neutral; //this is the original formula
			...
			if (speedLocked)
			{
				speedLocked = false; //currently this will allow for a new speed/accel to be stored, may change for future features
				cruiseDisplay.gameObject.SetActive(false);
			}
			//-------------------------------------------------
		}
		else
		{
			if (!speedLocked)
			{
				//first frame with CC on, store speed 
				cruiseAccel = Logitech.accel;
				cruiseSpeed = carControl.CurrentSpeed * 2.23693629f; //converting to mph in case we wish to display the current set speed on dashboard
				speedLocked = true;
				acceleration = cruiseAccel;
				cruiseDisplay.gameObject.SetActive(true);
				cruiseDisplay.text = ((int)(cruiseSpeed)).ToString();
			...
			{
				if (Logitech.keyboard)
				{
					if (Input.GetKey(KeyCode.LeftBracket))
					{
						cruiseSpeed -= Time.deltaTime * 4f;
					}
				...
				{
					if (Logitech.leftPaddle)
					{
						cruiseSpeed -= Time.deltaTime * 4f;
					}
					else if (Logitech.rightPaddle)
					{
						cruiseSpeed += Time.deltaTime * 4f;
				...
				Debug.Log(cruiseSpeed);
				//if (!(Logitech.leftPaddle & Logitech.rightPaddle))
				//{

				//}

				//accel and speed have been stored
				//this is where the fun begins
				float SpeedCurr = carControl.CurrentSpeed * 2.23693629f;
				float adjustAccel = (cruiseSpeed - SpeedCurr) / 500f; //make this 100f a variable later to test varying cruise control adjustment parameters
				cruiseAccel += adjustAccel;

				acceleration = cruiseAccel;
				cruiseDisplay.text = ((int)(cruiseSpeed)).ToString();
				/**
				if (SpeedCurr < cruiseSpeed ) //too slow
                {
					
                }
				else if(SpeedCurr > cruiseSpeed) //too fast
                {
					
                }
                else //currently matching
                {
					acceleration = cruiseAccel;
	...
	**/
		...
		//test accel as drive gear (comment out when normal accel test) --------------



		//acceleration =Logitech.accel *2* defaultAccel * neutral;
		//code for testing desired accel values
		Debug.Log(Logitech.accel);
		brake = Logitech.brake * Mathf.Clamp(CurrentSpeed_z, 1f, 10f) / 10f; //uncomment when doing normal test, remove the 2 when doing normal test

		//uncomment to use neutral gear as brake = 1
		//brake = neutralBrake;
		...
		//Simulate Physics
		carControl.Move(wheelTest + Logitech.wheel, acceleration, brakeTest + brake, 0, reverse, Logitech.park);
		...
		pitch_result += bodypitch + defaultPitch + CurrentSpeed_z / carControl.MaxSpeed * UnityEngine.Random.Range(-bumpyTestPitch, bumpyTestPitch) * Bumpyness;
		roll_result += bodyroll + defaultRoll + CurrentSpeed_z / carControl.MaxSpeed * UnityEngine.Random.Range(-bumpyTestRoll, bumpyTestRoll) * Bumpyness;
		...
		//original roll/pitch values = 20
		pitch_result = Mathf.Clamp(pitch_result, -70f, 70f);
		roll_result = Mathf.Clamp(roll_result, -120f, 120f);
	...
	public IEnumerator UpdateTelemetry()
	{
		...
		SimRacingStudio.SimRacingStudio_SendTelemetry(apiMode.ToCharArray()
														...
														, 0 //pmaxRpm
														, 0//int pgear
														, pitch_result * applyScale + pitchTest//ppitch
														, roll_result * applyScale + rollTest//proll
		, yaw_result * applyScale + yawTest//nothing this is yaw pyaw
		, 0//yaw but snapping at [-6,6] This registers as traction loss, plateral velocity
														, 0//roll, plateral acceleration
														, 0//pitch, pvertical acceleration
														, 0//pitch, plongitudinalAcceleration
														, 0//nothing, psuspensionTravelFrontLeft
														, 0//nothing, psuspensionTravelFrontRight
														, 0//nothing, psuspensionTravelRearLeft
														, 0//nothing,  psuspensionTravelRearRight
														, 0 //pwheelTerrainFrontLeft
														, 0 //pwheelTerrainFrontRight
														, 0 //pwheelTerrainRearLeft
														, 0 //pwheelTerrainRearRight
														);
